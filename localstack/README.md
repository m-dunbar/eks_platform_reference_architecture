...
- A container management service: while it could certainly be deployed in a cloud, with sufficient local memory and VCPUs, a local installation of Docker Desktop, Rancher Desktop or kubernetes provides the greatest utility
- installation of
  - localstack
  - aws cli
  - awslocal
...
Deployment and management scripts are provided in the top-level directory of the repo.
...
PLEASE NOTE: that the free tier of localstack does not provide persistence within the simulated local AWS environment, nor fully functional modeling of all services.  It does, however, allow for more robust initial testing of terraform HCL against the running localstack.
